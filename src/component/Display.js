...
const Display = (props) => {
  ...
  const handleChange = (search, option) => {console.log(search,option)};

  return (
    <>
      <Search handleChange={handleChange}/>
      <input style={{ width: "70%", margin: "10px", height: "25px" }} />
      <Button style={{ background: "#0275d8", padding: "5px", width: "7%" }}>
        Search
      </Button>
      ...
      <Dropdown isOpen={dropdownOpen} toggle={toggle}>
        ...
        <DropdownMenu>
          <DropdownItem onClick={() => setDropdownText("Search by first Name")}>
            First Name
          </DropdownItem>
          <DropdownItem onClick={() => setDropdownText("Search by Email")}>
            Email
          </DropdownItem>
        </DropdownMenu>
      </Dropdown>
